...
def calculate_dataset_hash(parent_dir: str) -> str:
    ...
    for pardir, _, files in os.walk(parent_dir):
        for file in files:
            ...
            relpath_file = os.path.relpath(file, pardir)
            true_files_hashes[relpath_file] = hash
            relpath_file = relpath_file.replace(os.path.sep, '$')
            hash = get_md5_adler32(bytes(relpath_file+hash, 'utf8'), True)
...
if __name__ == '__main__':
    your_path = '/home/klapeyron/lookalike_1/all_data/scoring_subset_v0'
    print(calculate_dataset_hash(your_path))
